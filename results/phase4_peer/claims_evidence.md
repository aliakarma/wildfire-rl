...
| P10 | **Generalization is measured by ΔWEL vs a same-condition No-Op** (falsifiable: No-Op = 0 by construction). Under held-out ignitions and shifted wind, CommNet and the strategic policies retain value (Saudi CommNet ΔWEL +17.6 held-out ignition, +24.6 wind+90); **but under rotated asset layouts the hierarchy and its heuristic teacher collapse to No-Op level** (Saudi ΔWEL +0.00/+0.08; California +0.04/+0.12), while egocentric policies (Local Reactive, CommNet) still adapt | phase5 generalization_summary_*.json |
| P11 | **Cross-region transfer is essentially absent** (the corrected experiment; the old "TRS>0.95 strong transfer" claim is false): a policy trained on the other region and run here scores ΔWEL ≤ +0.43 over No-Op in both directions — transfer neither helps nor is "robust" | phase5 generalization_summary_*.json (cross_region) |
| P12 | **Scaling (agent count):** the N=3-trained CommNet holds zero-shot at N∈{6,10} (Saudi WEL 11.4→12.3→9.6; California 19.2→16.6→19.4); Local Reactive improves monotonically with team size on scattered assets (California 15.1→8.8→5.2). The hierarchy is architecturally fixed to N=3 (per-agent commander heads) — a stated scaling limitation. Grid-size scaling (64²) is documented infeasible this pass (no 64² Cell2Fire landscape / infra rasters; all policies would need retraining) | phase5 scale_summary_*.json |
...
| "TRS > 0.95" / "strong cross-region generalization" | Metric vacuous + source run broken (Phase 0 H9); replaced by P10/P11 — corrected transfer shows ΔWEL ≤ +0.43 (essentially no transfer) |
| "Generalizes across fuel structures / infrastructure layouts" | Falsified by P10: hierarchy collapses to No-Op under rotated asset layouts |

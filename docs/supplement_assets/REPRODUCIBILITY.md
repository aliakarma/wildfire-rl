...
|---|---|---|
| Main table: WEL and ISR for 7 policies × 2 regions | 14 | `results/wildfire_phase3_multiseed/phase3_raw.csv` |
...
| Learned-commander ablation, incl. "never significantly improves" | 4 | `results/wildfire_phase3_hiercomm_learned/` |
| Extended difficulty sweep: best policy per (region, regime) cell | 6 | `results/wildfire_phase4_extended/robustness_results.csv` |
| Transfer matrix cells re-derived; TRS ratio identity; native diagonal matches main table | 3 | `results/wildfire_phase6/` |
...
cp -r results/wildfire_phase3_multiseed /tmp/repro
...
python - <<'PY'
import pandas as pd
frozen = pd.read_csv("results/wildfire_phase3_multiseed/phase3_raw.csv")
...
|---|---|---|
| Main results table (WEL/ISR, 7 policies × 2 regions) | Tier 1, or Tier 4 to retrain | `results/wildfire_phase3_multiseed/phase3_summary.json`, `phase3_main_table.tex` |
...
| Learned commander never significantly improves on the value-aware rule | Tier 1 | `results/wildfire_phase3_hiercomm_learned/phase3_summary.json` |
| Ablations (incl. communication is not the driver) | `python scripts/run_phase4_ablations.py --study ablation --ckpt-dir results/wildfire_phase3_multiseed --out /tmp/abl` | `results/wildfire_phase4/ablation_summary.json`, `phase4_ablation_table.tex` |
| Difficulty sweep: Local Reactive best on easy, HierComm best on medium/hard | Tier 1 | `results/wildfire_phase4_extended/robustness_results.csv` |
| Saudi↔California transfer matrix, TRS, CDGG, asymmetry | `python scripts/run_phase6_transfer.py --ckpt-dir results/wildfire_phase3_multiseed --out /tmp/transfer` | `results/wildfire_phase6/transfer_summary.json`, `phase6_transfer_table.tex` |
| Generalization sweep | `python scripts/run_phase6_transfer.py --study generalization --ckpt-dir results/wildfire_phase3_multiseed --out /tmp/gen` | `results/wildfire_phase6/generalization_summary.json` |

...
def draw_grid_cell(ax, y, x, color, alpha=1.0, marker=None, size=80, label=None):
    ...
    if marker:
        ax.scatter(x, y, color=color, marker=marker, s=size, edgecolors='black', zorder=5, label=label)
    else:
...
def render_rollout_frame(
    ...
    for i, (agent, pos) in enumerate(agent_positions.items()):
        ...
        if tar:
            ax.plot([pos[1], tar[1]], [pos[0], tar[0]], color=color, linestyle=':', alpha=0.6, linewidth=1.5)
    ...
    ax.text(
        ...
        bbox=dict(boxstyle='round,pad=0.4', facecolor='#F5F5F5', edgecolor='#E0E0E0', alpha=0.9),

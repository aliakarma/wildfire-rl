...
def plot_commander_decisions(
    ...
    for i, (agent, pos) in enumerate(agent_positions.items()):
        ...
        ax.scatter(pos[1], pos[0], color=color, marker='o', s=100, edgecolors='black', zorder=5, label=f"{agent} Start")
        
        if target:
            # Target center marker
            ax.scatter(target[1], target[0], color=color, marker='*', s=120, edgecolors='black', zorder=5)
            # Arrow from agent to target
            dy = target[0] - pos[0]
            dx = target[1] - pos[1]
            ax.arrow(
                pos[1], pos[0], dx * 0.8, dy * 0.8,
                head_width=1.0, head_length=1.2, fc=color, ec=color,
                alpha=0.8, length_includes_head=True, zorder=4
            )

...
(``wildfire_marl.data.CHANNEL_ORDER``) and the data card's normalization guarantee (per-channel
...
Any ``criticality.npy`` asset raster is checked for the same [0, 1] range.

Exit 0 = all OK, 1 = at least one invariant violated. CI-callable and part of the
provenance surface. These are the preserved V1 region tensors that feed the Phase-2
Cell2Fire landscape conversion.

Ported from V1 (``legacy_v1/scripts/validate_tensors.py``) in Phase 0; logic unchanged.
...
from wildfire_marl.data import CHANNEL_ORDER

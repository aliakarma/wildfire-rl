#!/usr/bin/env python
"""Train a single MARL V3 model from the command line.

Uses the V3 environment with leakage-free agent-caused suppression reward
and balanced scales.
"""
...
def main():
    ...
    ap.add_argument("--config", type=str, default="configs/ppo/marl_v3.yaml")
    args = ap.parse_args()
    ...
    def _factory():
        return MultiAgentWildfireEnvV3(
            state_tensor=tensor, config=env_cfg, num_agents=args.agents
        )

    # TensorBoard logging
    tb_log = str(ensure_dir(results_dir() / "runs" / "v3"))

    save_path = models_dir() / f"ppo_v3_marl_{args.region}_{args.agents}agents_seed_{args.seed}"

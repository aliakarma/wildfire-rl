...
def run_evaluation() -> None:
    ...
    for reg in regions:
        ...
        for n_agents in agent_counts:
            ...
            def make_marl_factory(num_a, tensor=tensor, env_cfg=env_cfg):
                return lambda: MultiAgentWildfireEnv(
                    state_tensor=tensor, config=env_cfg, num_agents=num_a
                )
            ...
            policies = {
                ...
                "nearest_fire": NearestFirePolicy(
                    sample_env.action_space, grid_size=32, env=sample_env
                ),
            ...
            for seed in seeds:
                model_path = (
                    models_dir() / f"ppo_marl_{region_name}_{n_agents}agents_seed_{seed}.zip"
                )
                    ...
                    raw_evals.append(
                        {
                            ...
                            - (ep["fire_intensity"] / (sample_env._initial_fire_total or 1.0)),
                        }
                    )
...
def run_detailed_diagnostics(
    ...
    model: Any,
    env_factory: Any,
    saudi_ppo_visits: dict,
    saudi_ppo_trajectories: dict,
    saudi_ppo_entropies: dict,
    saudi_ppo_repeated: dict,
...
def save_baseline_comparisons(df_raw: pd.DataFrame) -> None:
    ...
    pd.DataFrame(stats_rows).to_csv(
        ensure_dir(results_dir()) / "marl_baseline_statistics.csv", index=False
    )
    pd.DataFrame(effect_rows).to_csv(results_dir() / "marl_effect_sizes.csv", index=False)
    pd.DataFrame(controllability_rows).to_csv(
        results_dir() / "marl_controllability_metrics.csv", index=False
    )
    ...
    logger.info(
        "Wrote baseline stats, effect sizes, controllability, and scaling tables under results/"
    )


def save_coordination_metrics(
    saudi_ppo_visits: dict, saudi_ppo_repeated: dict, saudi_ppo_entropies: dict
) -> None:
    ...
    for n_agents in [1, 3, 5, 10]:
        ...
        for s in seeds:
            ...
            if n_agents == 1:
                ...
                visited_sets = [set(zip(*np.nonzero(vm), strict=False)) for vm in visit_maps]
                pairwise_overlaps = []
                for i in range(n_agents):
            ...
            rows.append(
                {
                    ...
                    "action_entropy_mean": np.mean(
                        [
                            saudi_ppo_entropies[(n_agents, s)]
                            for s in seeds
                            if (n_agents, s) in saudi_ppo_entropies
                        ]
                    ),
                    "repeated_action_ratio_mean": np.mean(
                        [
                            saudi_ppo_repeated[(n_agents, s)]
                            for s in seeds
                            if (n_agents, s) in saudi_ppo_repeated
                        ]
                    ),
                }
            )
...
def save_transfer_metrics(cfg: Any, seeds: list[int]) -> None:
    ...
    def saudi_factory():
        return MultiAgentWildfireEnv(state_tensor=saudi_tensor, config=env_cfg, num_agents=5)

    def california_factory():
        return MultiAgentWildfireEnv(state_tensor=california_tensor, config=env_cfg, num_agents=5)
    ...
    saudi_native = df_trans[
        (df_trans["train_region"] == "saudi") & (df_trans["test_region"] == "saudi")
    ]["reward_mean"].values[0]
    saudi_transfer = df_trans[
        (df_trans["train_region"] == "california") & (df_trans["test_region"] == "saudi")
    ]["reward_mean"].values[0]

    ca_native = df_trans[
        (df_trans["train_region"] == "california") & (df_trans["test_region"] == "california")
    ]["reward_mean"].values[0]
    ca_transfer = df_trans[
        (df_trans["train_region"] == "saudi") & (df_trans["test_region"] == "california")
    ]["reward_mean"].values[0]
    ...
    df_trans["transfer_degradation_pct"] = [
        ...
        for train, test in zip(df_trans["train_region"], df_trans["test_region"], strict=False)
    ]
...
def generate_figures(
    ...
    plt.title(
        "Saudi Arabia Wildfire Controllability Scaling Curves", fontsize=13, fontweight="bold"
    )
    plt.xticks([1, 3, 5, 10])
    plt.legend()
    ...
    fig.suptitle(
        "Spatial Suppression Visit Density Heatmaps (Saudi PPO)",
        fontsize=16,
        fontweight="bold",
        y=1.05,
    )
    ...
    plt.xlabel("Team Size (Number of Agents)", fontsize=12)
    plt.title(
        "PPO Policy Collapse Diagnostics vs Team Size (Saudi)", fontsize=13, fontweight="bold"
    )
    ...
    if (5, 0) in saudi_ppo_trajectories:
        plt.plot(
            saudi_ppo_trajectories[(5, 0)],
            label="PPO: 5 Agents (Seed 0)",
            color="blue",
            linewidth=2.5,
        )
    if (5, 1) in saudi_ppo_trajectories:
        plt.plot(
            saudi_ppo_trajectories[(5, 1)],
            label="PPO: 5 Agents (Seed 1)",
            color="cyan",
            linewidth=2.5,
        )
    if (5, 2) in saudi_ppo_trajectories:
        plt.plot(
            saudi_ppo_trajectories[(5, 2)],
            label="PPO: 5 Agents (Seed 2)",
            color="purple",
            linewidth=2.5,
        )
    ...
    if key in saudi_ppo_visits:
        ...
        plt.title(
            "Saudi PPO: Inter-Agent Spatial Visit Correlation Matrix (N=5)",
            fontsize=12,
            fontweight="bold",
        )
        plt.xlabel("Agent ID")
    ...
    if trans_stat_path.exists():
        ...
        plt.figure(figsize=(6, 5))
        sns.heatmap(
            pivot, annot=True, cmap="YlOrRd_r", fmt=".2f", cbar_kws={"label": "Normalized Reward"}
        )
        plt.title(
            "Cross-Regional Transfer Matrix Heatmap (5 Agents)", fontsize=13, fontweight="bold"
        )
    ...
    if trans_stat_path.exists():
        ...
        bars = plt.bar(
            df_deg["train_region"] + " -> " + df_deg["test_region"],
            df_deg["transfer_degradation_pct"],
            color=["orange", "darkred"],
        )
        plt.ylabel("Reward Degradation (%)", fontsize=12)
        plt.title(
            "Cross-Regional Transfer Degradation % (5 Agents)", fontsize=13, fontweight="bold"
        )
        plt.grid(axis="y", linestyle="--", alpha=0.6)
    ...
    if key3 in saudi_ppo_agent_trajectories:
        ...
        for idx, path in enumerate(paths):
            ...
            plt.scatter(
                ys[0], xs[0], color=colors[idx], marker="s", s=60, edgecolors="black", zorder=5
            )

        plt.title(
            "Agent Spatial Path Trajectories: Division of Labor (N=3)",
            fontsize=12,
            fontweight="bold",
        )
    ...
    if coord_path.exists():
        ...
        ax1.plot(
            df_coord["num_agents"],
            df_coord["coverage_efficiency_mean"],
            marker="s",
            color="darkgreen",
            linewidth=2.5,
            label="Unique Coverage per Step",
        )
        ...
        ax2.plot(
            df_coord["num_agents"],
            df_coord["region_specialization_mean"],
            marker="^",
            color="darkblue",
            linewidth=2.5,
            label="Region Specialization",
        )
        ax2.set_ylabel("Region Specialization Metric", color="darkblue", fontsize=12)
        ax2.tick_params(axis="y", labelcolor="darkblue")

        plt.title(
            "Coordination Efficiency & Spatial Specialization vs Team Size",
            fontsize=12,
            fontweight="bold",
        )
        fig.tight_layout()
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--timesteps", type=int, default=100000, help="Timesteps to train missing PPO models"
    )
    args = ap.parse_args()

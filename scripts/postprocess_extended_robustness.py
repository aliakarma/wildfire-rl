...
The frozen 4-policy robustness artifact (``wildfire_phase4/``) was built by resuming from a
...
This script rewrites ``wildfire_phase4_extended/robustness_summary.json`` so that every
policy's ``medium`` cell equals its frozen main-results (15-episode) WEL/ISR mean from
``wildfire_phase3_multiseed/phase3_summary.json`` — the identical environment condition,
...
Run after ``run_phase4_ablations.py --study robustness --out wildfire_phase4_extended``.
"""
...
EXT = REPO / "wildfire_phase4_extended"

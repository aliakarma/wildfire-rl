...
from wildfire_marl.eval.statistics import compare_policies, compute_summary_stats
...
def run_certification(
    ...
    # 1. Generate Manifest
    manifest_path = generate_reproducibility_manifest(
        out_path, region, base_seed, hierarchical_ckpt, flat_ckpt
    )
    ...
    if run_multiseed or not raw_csv.exists():
        print("Executing multi-seed evaluation pipeline...")
        import subprocess
    ...
    if len(df_seed["seed"].unique()) < 2:
        print(
            "WARNING: Less than 2 seeds evaluated. Using episode-level samples for statistical calculations."
        )
    ...
    summary_report = {"region": region, "metrics": {}, "comparisons": {}}
    ...
    plot_sector_occupancy(
        compliance_report["sector_occupancy"], out_path / f"sector_occupancy_{region}.png"
    )
    ...
    if hier_wel_hi < vf_wel_lo:
        passed = True
        reasons.append(
            "Condition A: Hierarchical significantly beats Value-First on WEL (CIs do not overlap)."
        )
    if hier_isr_lo > vf_isr_hi:
        passed = True
        reasons.append(
            "Condition A: Hierarchical significantly beats Value-First on ISR (CIs do not overlap)."
        )
    ...
    summary_report["comparisons"]["WEL"]["Flat_MARL"]["p_val"]
    ...
    if wel_match and isr_match and (hier_wel_mean < flat_wel_mean):
        passed = True
        reasons.append(
            "Condition B: Hierarchical statistically matches Value-First and outperforms Flat MARL."
        )
    ...
    if wel_within_10 and isr_within_10 and (hier_ce >= vf_ce):
        passed = True
        reasons.append(
            "Condition C: Near-parity (within 10%) achieved with superior/equal coordination efficiency."
        )
    ...
    if not passed:
        reasons.append(
            "FAIL: Policy underperforms heuristic significantly and shows no separation from Flat MARL."
        )
...
def main():
    ...
    parser.add_argument(
        "--skip_multiseed", action="store_true", help="Skip running multiseed script again"
    )

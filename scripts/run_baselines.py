...
from wildfire_marl.agents.ppo_baseline import (
    SB3PredictorWrapper,
    train_ppo,
)
...
def run_benchmark(
    ...
    for region in regions:
        ...
        def env_factory(map_name=map_name):
        ...
        for name, policy in policies.items():
            ...
            summary = res["summary"]
            print(
                f"  Reward: {summary['episode_reward_mean']:.2f} ± {summary['episode_reward_std']:.2f}"
            )
            print(
                f"  Burned Cells: {summary['burned_cells_mean']:.2f} ± {summary['burned_cells_std']:.2f}"
            )
            if "wel_mean" in summary:
                print(
                    f"  Economic Loss (WEL): {summary['wel_mean']:.2f} ± {summary['wel_std']:.2f}"
                )
                print(
                    f"  Survival Rate (ISR): {summary['isr_mean']:.2f} ± {summary['isr_std']:.2f}"
                )
        ...
        for name, res in eval_results.items():
            ...
            t_test = (
                welch_ttest(ep_rewards, no_op_rewards)
                if name != "No-Op"
                else {"t_statistic": 0.0, "p_value": 1.0, "cohens_d": 0.0}
            )
            ...
            wel_ci = (
                bootstrap_ci(ep_wel, seed=base_seed) if "wel" in res["episodes"][0] else (0.0, 0.0)
            )
            ...
            all_summary_rows.append(
                {
                    ...
                    "sig_stars": (
                        format_significance(t_test["p_value"]) if name != "No-Op" else "n/a"
                    ),
                }
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--region", default="both", choices=["saudi", "california", "both"], help="Region."
    )
    parser.add_argument("--episodes", type=int, default=20, help="Number of evaluation episodes.")
    parser.add_argument(
        "--train-timesteps", type=int, default=10000, help="Timesteps to train Maskable-PPO."
    )

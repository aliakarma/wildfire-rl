...
from wildfire_marl.eval.transfer import transfer_robustness_score, adaptation_asymmetry, cross_domain_gap
...
def evaluate_transfer_cell_v2(
    ...
    for ep in range(num_episodes):
        ...
        metrics, _, _ = run_episode_eval(
            env, setting, commander, flat_marl_actor, seed, device
        )
...
def main():
    ...
    for method in methods:
        ...
        res_s_s = evaluate_transfer_cell_v2(env_saudi, method, ckpt_saudi, args.episodes, args.seed, device)
        
        # 2. Saudi on California (S -> C, Transfer)
        print("Evaluating Saudi Policy on California Environment...")
        res_s_c = evaluate_transfer_cell_v2(env_cali, method, ckpt_saudi, args.episodes, args.seed, device)
        
        # 3. California on California (C -> C, Native)
        print("Evaluating California Policy on California Environment...")
        res_c_c = evaluate_transfer_cell_v2(env_cali, method, ckpt_cali, args.episodes, args.seed, device)
        
        # 4. California on Saudi (C -> S, Transfer)
        print("Evaluating California Policy on Saudi Environment...")
        res_c_s = evaluate_transfer_cell_v2(env_saudi, method, ckpt_cali, args.episodes, args.seed, device)

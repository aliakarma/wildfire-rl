...
Example::
    ...
    python scripts/render_phase6_transfer.py --ckpt-dir results/wildfire_phase3_multiseed \\
...
def main() -> None:
    ...
    ap.add_argument(
        "--seed", type=int, default=42, help="Training-seed checkpoint + eval seed group"
    )
    ...
    for eval_region in regions:
        ...
        renderer = GeoRenderer(
            eval_region, style=args.style, data_dir=args.data_dir, theme=args.theme
        )
        ...
        for kind in policies:
            ...
            native_nets = load_nets(
                kind, eval_region, args.ckpt_dir, env.num_agents, device, seed=args.seed
            )
            ...
            nat_frames, nat_scores = rollout_frames(
                ...
                args.seed,
                eval_region,
                f"{base}  [native {REGION_ABBR[eval_region]}]",
                args.max_steps,
            )
            # transferred: trained on the other region, evaluated here
            xfer_nets = load_nets(
                kind, train_region, args.ckpt_dir, env.num_agents, device, seed=args.seed
            )
            renderer._fire_history.clear()
            ...
            compile_rollout_gif(
                nat_frames,
                out_dir / f"rollout_{kind}_{eval_region}_native.gif",
            ...
            _pair_snapshot(
                nat_frames[ni], xfer_frames[xi], out_dir / f"fig_transfer_{kind}_{eval_region}.png"
            )
            print(
                f"  {kind}: native({len(nat_frames)}f) + transfer({len(xfer_frames)}f) "
                f"-> {eval_region}",
                flush=True,
            )
        env.close()
